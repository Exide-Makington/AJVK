...
func walk(delta):
	...
	if (direction.length() > 0 and $StairCatcher.is_colliding() and is_on_floor()):
	...
	var view_bob = (OS.get_ticks_msec()/150.0)
	$Head/Camera.h_offset = sin(view_bob)/1.6*pow((h_vel.length()/MAX_RUNNING_SPEED),2)
	$Head/Camera.v_offset = cos(2*view_bob)/4*(h_vel.length()/MAX_RUNNING_SPEED)
	print(h_vel.length()/speed)
